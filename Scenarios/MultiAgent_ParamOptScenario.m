    ...
    methods
        ...
        function plotResults(obj)
            ...
            for i = 1:obj.n_agents
                ...
                for k = 1:t_len
                    u = obj.ctrl(2*k-1:2*k,1);
                    [v_vec(:,k), w_vec(:,k)] = obj.agents(i).vehicle.kinematics.getVelocities(obj.tmat(k), obj.xmat(:,k,i+1), u(:,1));
                    ...
                    err_vec(:,k) = norm([obj.agents(i).trajectory.x(k+1);obj.agents(i).trajectory.y(k+1)] - obj.xmat(1:2,k,i+1));

                    h_e = [cos(obj.agents(i).trajectory.psi(k+1));sin(obj.agents(i).trajectory.psi(k+1))] - [cos(obj.xmat(3,k,i+1));sin(obj.xmat(3,k,i+1))];
                    yaw_err_vec(:,k) = h_e'*h_e/2;
                end
